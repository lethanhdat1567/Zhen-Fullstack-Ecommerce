...
const postCategoryColumns = ({ router, onRefreshs }: Props) => [
    createSelectColumn(),
    ...
    // ===== STATUS =====
    {
        ...
        cell: ({ row }: { row: any }) => (
            <ToggleStatus
                ...
                onChange={async () => {
                    try {
                        await postCategoryService.toggleStatus(row.original.id);
    ...
    createActionsColumn<MediaCategoryType>({
        onUpdate: (row: any) => {
            router.push(`/admin/posts/category/${row.id}`);
        },
        onDelete: async (row) => {
            try {
                await postCategoryService.delete(row.id);
...
export default postCategoryColumns;
