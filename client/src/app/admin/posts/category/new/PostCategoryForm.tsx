...
interface Props {
    activeLang: "vi" | "en" | "fr";
    form: ReturnType<typeof useForm<z.infer<typeof postCategorySchema>>>;
    updateId?: string;
}

function PostCategoryForm({ activeLang, form, updateId }: Props) {
    ...
    async function onSubmit(data: z.infer<typeof postCategorySchema>) {
        try {
            if (updateId) {
                await postCategoryService.update(updateId, data);
                toast.success("Cập nhật danh mục thành công!");
            } else {
                await postCategoryService.create(data as any);
                toast.success("Tạo danh mục thành công!");
            }

            router.push("/admin/posts/category" as any);
        } catch (error) {
