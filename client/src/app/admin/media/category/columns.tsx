...
const mediaCategoryColumns = ({ router, onRefreshs }: Props) => [
    createSelectColumn(),
    ...
    // ===== STATUS =====
    {
        ...
        cell: ({ row }: { row: any }) => (
            <ToggleStatus
                ...
                onChange={async () => {
                    try {
                        await mediaCategoryService.toggleStatus(
                            row.original.id,
                        );
    ...
    createActionsColumn<MediaCategoryType>({
        onUpdate: (row: any) => {
            router.push(`/admin/media/category/${row.id}`);
        },
        onDelete: async (row) => {
            try {
                await mediaCategoryService.delete(row.id);
...
export default mediaCategoryColumns;
